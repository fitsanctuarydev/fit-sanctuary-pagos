...
echo "📧 Agregando sistema de correos SMTP..."

# 1. Instalar Nodemailer en el Backend
npm install nodemailer

# 2. ACTUALIZAR SERVER.JS (Con lógica de envío de correos)
cat > server.js <<'EOF'
...
# 3. ACTUALIZAR FRONTEND (APP.JSX) CON CAMPO DE EMAIL
...
if (MP_KEY && !MP_KEY.includes('TU_CLAVE')) { initMercadoPago(MP_KEY, { locale: 'es-MX' }); }
...
function App() {
    ...
    const initPayment = async (method, plan) => {
        if (!email || !email.includes('@')) {
            alert("Por favor ingresa un correo electrónico válido para enviarte tu recibo.");
            return;
        }
        ...
        <div className="min-h-screen bg-[#0f0f0f] text-white font-sans pb-20 selection:bg-yellow-500 selection:text-black">
            ...
            <main className="pt-24 px-4 max-w-xl mx-auto relative z-10">
                ...
                {view === 'checkout' && selectedPlan && (
                    <div className="animate-fade-in">
                        <button onClick={() => { setView('shop'); setPaymentMethod(null); }} className="text-xs text-neutral-500 mb-6 hover:text-white flex items-center gap-1"><ArrowLeft className="w-3 h-3" /> Cambiar plan</button>
                            ...
                            <div className="animate-fade-in">
                                ...
                                {paymentMethod === 'mp' && mpPreferenceId && (
                                    <div className="bg-white rounded-lg p-4">
                                        <Payment initialization={{ amount: getTotal(selectedPlan.price), preferenceId: mpPreferenceId }} customization={{ visual: { style: { theme: 'default' } } }} onSubmit={handleSuccess} />
                                    </div>
                                )}
...
echo "✅ Correos SMTP configurados."
echo "👉 Para subir: git add . && git commit -m 'Add Email' && git push origin main"
```

3.  Ejecuta `bash add_email.sh`.
4.  Sube los cambios:
    ```bash
    git add . && git commit -m "Add Email" && git push origin main