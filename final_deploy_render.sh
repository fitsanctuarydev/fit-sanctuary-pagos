...
echo "🚀 Preparando versión final para Render (Producción)..."

# 1. ASEGURAR DEPENDENCIAS DE DESARROLLO EN BUILD (Vital para Render)
# Render pone NODE_ENV=production por defecto, lo que salta 'vite'. Esto lo fuerza.
cat > package.json <<'EOF'
{
  "name": "fit-sanctuary-api",
  "version": "1.0.0",
  "description": "Backend pagos",
  "main": "server.js",
  "scripts": {
...
# 2. SERVER.JS (Backend + Servidor de Archivos)
cat > server.js <<'EOF'
...
# 3. ACTUALIZAR APP.JSX (Frontend con corrección MP definitiva)
# IMPORTANTE: Eliminamos 'amount' del Payment Brick para evitar el error 400
...
// Inicialización segura de MP
if (MP_KEY && !MP_KEY.includes('TU_CLAVE')) { 
    try { initMercadoPago(MP_KEY, { locale: 'es-MX' }); } catch(e) { console.error("Error init MP", e); }
}
    ...
    const initPayment = async (method, plan) => {
        ...
        try {
            ...
            if (method === 'mp') {
                ...
                // Reset y asignar para forzar re-render
                setMpPreferenceId(null);
                setTimeout(() => setMpPreferenceId(data.id), 50);
            }
        } catch (e) {
            console.error(e);
    ...
    return (
        <div className="min-h-screen bg-[#0f0f0f] text-white font-sans pb-20 selection:bg-yellow-500 selection:text-black">
            ...
            <main className="pt-24 px-4 max-w-xl mx-auto relative z-10">
                {/* VISTAS (TIENDA Y CHECKOUT IDÉNTICAS A ANTES) */}
                {view === 'shop' && (
                    ...
                    <div className="animate-fade-in">
                        ...
                        {!paymentMethod ? (
                            ...
                            <div className="animate-fade-in">
                                ...
                                {/* MERCADO PAGO BRICK SIN AMOUNT (CORRECCIÓN CRÍTICA) */}
...
echo "✅ Código listo para Render."
echo "👉 1. Guarda el archivo."
echo "👉 2. Ejecuta: bash final_deploy_render.sh"
echo "👉 3. Ejecuta: git add . && git commit -m 'Release final' && git push origin main"